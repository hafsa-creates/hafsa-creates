# Hi there, I'm Hafsa Mahek 
[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=26&pause=1000&color=00FF99&center=true&vCenter=true&width=900&lines=Cybersecurity+Student;Blue+Team+Enthusiast;DFIR+Intern;SOC+%7C+Threat+Intelligence;Always+Learning+Something+New)](https://git.io/typing-svg)
![Cybersecurity](https://img.shields.io/badge/Cybersecurity-Professional-red?style=for-the-badge)
![Blue Team](https://img.shields.io/badge/Blue-Team-blue?style=for-the-badge)
![DFIR](https://img.shields.io/badge/DFIR-orange?style=for-the-badge)
![SOC](https://img.shields.io/badge/SOC-Analyst-success?style=for-the-badge)
![](https://komarev.com/ghpvc/?username=hafsa-creates&color=blue)
### BS Cybersecurity Student | DFIR Intern | Blue Team & Cyber Threat Intelligence Enthusiast

I'm a 4th-semester BS Cybersecurity student at Air University (GPA: **3.72/4.00**) currently working as a **Digital Forensics & Incident Response (DFIR) Intern** at the National Cyber Crime Investigation Agency (NCCIA). I enjoy building practical cybersecurity labs focused on SOC operations, cyber threat intelligence, detection engineering, and network defense.

---

## 👩‍💻 About Me

* 🎓 BS Cybersecurity Student at Air University
* 🔎 DFIR Intern at NCCIA
* 🛡️ Blue Team & SOC Enthusiast
* 🎯 Interested in Cyber Threat Intelligence (CTI), Detection Engineering, DFIR, and Incident Response
* 📚 Continuously building hands-on cybersecurity labs and automation projects

---

## 🛠 Technical Skills
## 💻 Technologies & Tools

<p align="left">
<img src="https://skillicons.dev/icons?i=python,bash,cpp,linux,git,github,vscode"/>
</p>
### SIEM & Detection

* Splunk
* Wazuh
* Sysmon
* Log Analysis
* YARA Rules
* Incident Response

### CTI & Threat Intelligence

* MITRE ATT&CK
* STIX/TAXII
* ThreatConnect
* AlienVault OTX
* IBM X-Force
* VirusTotal
* Valkyrie Sandbox

### Penetration Testing & Network Security

* Kali Linux
* Metasploit
* Burp Suite
* Nmap
* Hydra
* Wireshark
* OWASP

### Programming

* Python
* C++
* Bash

---

## 🚀 Featured Projects

### 🛡️ Sentinel – Cyber Threat Intelligence Platform

Threat intelligence platform integrating multiple threat feeds, custom YARA detection, STIX automation, and MITRE ATT&CK mapping.

### 📊 Splunk Threat Detection Lab

Security monitoring lab using Splunk, Sysmon, and PowerShell logs to detect brute-force attacks, malicious PowerShell execution, and USB activity.

### 🔐 Wazuh SIEM Implementation

Centralized log collection, attack simulation, NIST control mapping, and real-time alerting across multiple systems.

### 📧 Phishing Detection Tool

Python-based email analysis tool that inspects headers and URLs using the VirusTotal API and pattern matching to identify phishing indicators.

### 🌐 Cloud-Based Network IDS

Cloud-hosted intrusion detection environment with custom IDS rules, honeypot integration, and traffic analysis.

### 🍯 Cowrie Honeypot

SSH honeypot integrated with Wazuh for attacker behavior analysis and MITRE ATT&CK mapping.

---

## 📖 Currently Learning

* Digital Forensics
* Detection Engineering
* Malware Analysis
* Threat Hunting
* Microsoft Sentinel
* Azure Security

---

## 📜 Certifications

Currently pursuing practical cybersecurity training through hands-on labs, TryHackMe, Hack The Box, and Microsoft Learn.

---

## 🌍 Connect With Me

📧 Email: **[hafsahmaher7@gmail.com](mailto:hafsahmaher7@gmail.com)**

💼 LinkedIn: **linkedin.com/in/hafsa-mahek**

📍 Multan, Pakistan
## 📊 GitHub Statistics

![Hafsa's GitHub Stats](https://github-readme-stats.vercel.app/api?username=hafsa-creates&show_icons=true&theme=tokyonight)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=hafsa-creates&layout=compact&theme=tokyonight)

---

> *"Learning cybersecurity by building, breaking, defending, and documenting real-world labs."*

